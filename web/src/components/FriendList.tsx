...
export const FriendList = observer(() => {
  ...
  useEffect(() => {
    ...
    rootStore.messageStore.fetchFirstMessages();
  }, [rootStore.messageStore]);

  const asdf = () => {
    console.log(rootStore.messageStore.messages.values().next());
  };
        ...
        <List>
          {
            <>
              {asdf()}
              {console.log(rootStore.messageStore.messages)}
              <div>p</div>
              <div>{rootStore.messageStore.messages.size}</div>
            </>
          }
  ...
  return (
    <MyScrollbar autoHide noScrollX>
      <Container>
        {isBg && <SearchBar />}
        <List>
          {store.messageStore.messages.values.map((message) => (
            <Item>
              <Avatar src="https://scontent-frt3-1.xx.fbcdn.net/v/t1.30497-1/c29.0.100.100a/p100x100/84241059_189132118950875_4138507100605120512_n.jpg?_nc_cat=1&_nc_sid=7206a8&_nc_ohc=zLcWETlL6TIAX95SnPH&_nc_ht=scontent-frt3-1.xx&_nc_tp=27&oh=062a10092c5cb258d651a46799cf7c89&oe=5FA0C29E"></Avatar>
              <Username>{message.sender.username}</Username>
              <Message>{message.text}</Message>
